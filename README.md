## Hi there I am kortay

![a fox](https://www.pinterest.com/pin/990017930585722904/)

Software Developer and Game Designer. I like making my imagination into reality.


**Currently:** building a first person shooter on the go.

**Learning:** AI optimisation for path finding.

---

### Proudest Works
- **[Dungeon Generator](https://github.com/KortayKivanc/DungeonGeneratorAlgorithm):** Created an algorithm that can generate completely random and traversable, connected dungeons using also highly customizable assets that the user can create on their own.

- **[USV Ground Controller](https://cakubarbaros.com/duyuru-detail.php?id=yer-kontrol-istasyonu-v3-2):** Led a team of 4 people that created a Ground Control Station for a USV that made high scores (91/100) in yearly Teknofest.

- **Gesture Drawing App**: Made a gesture drawing app of my own for my art studies, made sure it was useful for training artistic skills. So far 50+ people used it and average rating I get was around 4.5/5. After 2 years most of them are still using the said app.

---

**Looking for:** [ Job/Internship](https://www.linkedin.com/in/kortaykokturk/)


